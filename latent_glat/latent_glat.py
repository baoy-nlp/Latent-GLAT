...
from .glat import init_bert_params, GlancingTransformerDecoder, GlancingTransformer
from .utils import GateNet
from .vector_quantization import vq_st, vq_search

INF = 1e-10


@register_model("latent_glat")
class LatentGLAT(GlancingTransformer):
    """
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        decoder = LatentGLATDecoder(args, tgt_dict, embed_tokens)
    ...
    def add_args(parser):
        ...
        LatentGLATDecoder.add_args(parser)
...
class LatentGLATDecoder(GlancingTransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        super().__init__(args, dictionary, embed_tokens, no_encoder_attn)

        # add after attending to 2021-05-07
        self.condition_on_z = getattr(args, "condition_on_z", False)
        self.share_bottom_layers = getattr(args, "share_bottom_layers", False)

        self.glat_training = getattr(args, "glat_training", False)
        self.gate = GateNet(
            d_model=self.embed_dim * 2, d_hidden=self.embed_dim * 4, d_output=self.embed_dim, dropout=args.dropout
        ) if getattr(args, "gated_func", "residual") == "residual" else None

        if args.code_cls == "ema":
            self.code = EMACode(num_codes=args.num_codes, code_dim=args.decoder_embed_dim, lamda=args.lamda)
        elif args.code_cls == "soft":
            self.code = SoftCode(num_codes=args.num_codes, code_dim=args.decoder_embed_dim)
        else:
            raise RuntimeError("code cls:{} is error".format(args.code_cls))
        ...
        self.latent_predictor = self._build_latent_predictor()
    ...
    def extract_features(
        ...
        if tgt_tokens is not None and self.glat_training and self.training:
            ...
            decoder_inputs, predict, observed = self.glancing_for_decode(
                features=features,
                targets=tgt_tokens,
                mask=decoder_padding_mask + z_ret["prior"]["observed"] if self.condition_on_z else decoder_padding_mask,
            ...
            # TODO: remove str as hard mapping keys
    ...
    def forward_z(self, encoder_out, tgt_tokens=None, inputs=None, decoder_padding_mask=None):
        if tgt_tokens is not None:
            ...
            posterior, idx = self._inference_z(inputs, decoder_padding_mask, tgt_tokens)
        else:
            posterior, idx = None, None

        if self.latent_predictor is None:
            # a non-parameterize predictor, nearest search with decoder inputs
            prior, idx = self._inference_z(inputs, decoder_padding_mask)
        else:
            # a parameterize predictor, we use GLAT here.
            prior, idx = self._predict_z(inputs, decoder_padding_mask, encoder_out, tgt=idx, out=posterior)
        ...
        return q, {"prior": prior, "posterior": posterior}
    ...
    def step(self, step_num):
        super().step(step_num)
        self.latent_predictor.step(step_num)
    ...
    def _predict_z(self, inputs, decoder_padding_mask, encoder_out, tgt=None, out=None):
        ...
        ret = self.latent_predictor.forward_as_submodule(inputs, decoder_padding_mask, encoder_out, tgt_tokens=tgt)
        ...
        if tgt is None:
            ...
            # schedule sampling / explore oracle analysis
            if self.vq_schedule and self.training:
                # TODO:  to input the latent codes, instead of mix decoder inputs and latent codes
                z_mix = ext["glancing_input"]

                if self.vq_mask_same:
                    z_mix = self.latent_predictor.sampler.forward_inputs(
                        inputs=None, ref=self.latent_predictor.forward_embedding(tgt)[0],
                        observed=ext["ref_mask"], s_mode="schedule",
                        pred=self.latent_predictor.forward_embedding(ext["predict"])[0],
                    )
                elif self.vq_mask_reverse:
                    z_mix = self.latent_predictor.glancing_for_decode(
                        features=ext["features"], targets=tgt,
                        mask=ext["ref_mask"].squeeze(-1) > 0. + decoder_padding_mask,
                        ratio=self.vq_schedule_ratio, inputs=inputs
                    )[0]
                elif self.vq_mix_diff:
                    z_mix = self.latent_predictor.glancing_for_decode(
                        features=ext["features"], targets=tgt, mask=decoder_padding_mask,
                        ratio=self.vq_schedule_ratio, inputs=inputs,
                    )[0]
            else:
                z_mix = out["z_q_st"]
            ...
            return {
                       "VQ": {  # used for latent predictor
                           "out": z_out, "tgt": tgt, "factor": self.latent_factor,
                           "mask": ~(decoder_padding_mask + ext["ref_mask"].squeeze(-1) > 0.)
                       },
                       "observed": ext["ref_mask"].squeeze(-1) > 0.,  # glancing reference latent variables
                       "z_input": z_mix,  # mixed outputs
                   }, z_idx

    def _build_latent_predictor(self):
        main_args = self.args
...
@register_model_architecture("latent_glat", "latent_glat_wmt14")
def wmt14_en_de(args):
    base_architecture(args)


@register_model_architecture("latent_glat", "latent_glat_iwslt16")
...
@register_model_architecture("latent_glat", "latent_glat_iwslt14")
...
@register_model_architecture("latent_glat", "latent_glat_base")
def nat_base(args):
